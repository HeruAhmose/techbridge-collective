...
function Get-ProductionUrl {
  ...
  $line = ($raw -split "`n") | Where-Object { $_ -match "https://.*\.vercel\.app" -and $_ -match "Production" } | Select-Object -First 1
  if (-not $line) { $line = ($raw -split "`n") | Where-Object { $_ -match "https://.*\.vercel\.app" } | Select-Object -First 1 }
  $m = [regex]::Match($line, "https://[a-zA-Z0-9\-]+\.vercel\.app")
  if (-not $m.Success) { throw "Could not parse production URL" }
  return $m.Value
}
...
if ($Secret) {
  ...
  try { Invoke-WebRequest -Uri "$BaseUrl/?x-vercel-protection-bypass=$Secret&x-vercel-set-bypass-cookie=samesitenone" -WebSession $session -MaximumRedirection 10 -SkipHttpErrorCheck -TimeoutSec 15 | Out-Null } catch {}
}
...
foreach ($t in $targets) {
  ...
  $title = if ($resp -and $resp.Content) { $tstr = Get-PageTitle $resp.Content; if ($tstr) { "  [$tstr]" } else { "" } } else { "" }
...
Write-Host ""
if ($fail -eq 0) { Write-Host "  ALL $pass/$pass PASSED" -ForegroundColor Green }
else { Write-Host "  $pass passed / $fail FAILED — run: vercel logs --prod" -ForegroundColor Red; exit 1 }
