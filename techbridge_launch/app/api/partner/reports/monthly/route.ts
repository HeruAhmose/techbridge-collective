...
export async function GET(req: NextRequest) {
  ...
  const hubs = await prisma.hubLocation.findMany({ where: { partnerOrgId: orgId } });
  const hubIds = hubs.map((h) => h.id);
  const { start, end } = monthRange(month);

  const where = { createdAt: { gte: start, lt: end }, hubLocationId: { in: hubIds.length ? hubIds : ["__none__"] } };
  ...
  if (format === "csv") {
    ...
    return new Response(toCsv(rows), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="partner-${org.name.replace(/\s+/g, "-").toLowerCase()}-${month}.csv"`,
  ...
  doc.fontSize(20).text("TechBridge Collective — Partner Report", { align: "left" });
  doc.moveDown(0.5);
  doc.fontSize(12).fillColor("#444444").text(`Organization: ${org.name}`, { align: "left" });
  ...
  if (byCategory.length) {
    byCategory.slice(0, 8).forEach((c) => {
      doc.fontSize(11).text(`• ${c.category}: ${(c._sum.minutes ?? 0).toLocaleString()} minutes (${c._count._all} sessions)`);
    });
  } else {
  ...
  doc.moveDown(0.5);
  if (byHub.length) {
    byHub.slice(0, 10).forEach((h) => {
      const name = hubName.get(h.hubLocationId) || h.hubLocationId;
      doc.fontSize(11).text(`• ${name}: ${(h._sum.minutes ?? 0).toLocaleString()} minutes (${h._count._all} sessions)`);
    });
  } else {
    doc.fontSize(11).fillColor("#666").text("No data yet.").fillColor("#000");
  }
