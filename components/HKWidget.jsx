...
// Inject CSS via useEffect to avoid Next.js SSR hydration mismatch
const HK_STYLE_ID = "hk-widget-styles";
function useHKStyles() {
  useEffect(() => {
    if (typeof document === "undefined") return;
    if (document.getElementById(HK_STYLE_ID)) return;
...
// ── CHAT PANEL ─────────────────────────────────────────────────
function Panel({ engine, compact=false, showHead=true, onClose=null }) {
      ...
      {showHead && (
        <div style={{padding:"11px 14px",borderBottom:`1px solid ${C.bd}`,display:"flex",justifyContent:"space-between",alignItems:"center",background:`linear-gradient(135deg,${C.cyanA},transparent)`,flexShrink:0}}>
          ...
          <div style={{display:"flex",gap:5,alignItems:"center"}}>
            ...
            {onClose&&<button onClick={onClose} aria-label="Close chat" className="hk-r" style={{background:"transparent",border:`1px solid ${C.bd}`,borderRadius:5,width:24,height:24,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",transition:"all .15s",marginLeft:2}}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={C.tx2} strokeWidth="2.5" strokeLinecap="round"><path d="M18 6L6 18"/><path d="M6 6l12 12"/></svg>
            </button>}
          </div>
        </div>
      )}
    ...
    <>
      <div style={{position:"fixed",bottom:20,right:20,zIndex:9999,fontFamily:F}}>
        ...
        <div style={{
          ...
          {(open||opened) && <Panel engine={engine} compact={true} onClose={()=>setOpen(false)}/>}
        </div>
    ...
    <>
      {!open ? (
        ...
        <div className="hk-pop" style={{background:C.bg1,borderRadius:14,border:`1px solid ${C.bd}`,overflow:"hidden",height:440,display:"flex",flexDirection:"column",boxShadow:"0 8px 32px rgba(0,0,0,.45)",width:"100%",maxWidth:420}}>
          <Panel engine={engine} compact={true} onClose={()=>setOpen(false)}/>
        </div>
      )}
    </>
  );
